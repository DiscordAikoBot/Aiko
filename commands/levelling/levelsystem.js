      ...
      const type = args[0];
            ...
            switch(type) {
                ...
                case "toggle":
                    ...
                    if(!toggle) return message.channel.send({embed: {color:'#eb0936', description:"<:tick_no:746298075643117620> Please mention on/off to turn on and off the level system respectively."}})
                    message.channel.send({embed: {color:'#10de47', description:`<:tick_yes:746298071951867906> The leveling system has been turned **${toggle.toUpperCase()}**.`}});
                ...
                case "channel":
                    ...
                    if(!checking || checking === 'off') return message.channel.send({embed: {color:'#eb0936', description:"<:tick_no:746298075643117620> Please toggle **ON** the level feature."}});
                    if(!ch) return message.channel.send({embed: {color:'#eb0936', description:"<:tick_no:746298075643117620> Please enter the command responce."}})
                ...
                case "del-channel":
                    message.channel.send({embed: {color:'#10de47', description:`<:tick_yes:746298071951867906> Sucessfully deleted the **Level Message Channel** for the server ${message.guild.name}`}});
                ...
                case "message":
                    ...
                    if(!checking || checking === 'off') return message.channel.send({embed: {color:'#eb0936', description:"<:tick_no:746298075643117620> Please toggle **ON** the level system."}});
                     ...
                     const contentembed = new Discord.MessageEmbed()
                          .setColor('#eb0936')
                          .setDescription("<:tick_no:746298075643117620> Please add a level message along with the tags for the level up message!")
